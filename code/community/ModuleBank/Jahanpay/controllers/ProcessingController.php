 ...
 */
class ModuleBank_Jahanpay_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType  = 'jahanpay/success';
    protected $_failureBlockType  = 'jahanpay/failure';
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage::helper('jahanpay')->__('Customer was redirected to jahanpay gateway.')
                )->save();
            }
	...
	{
		$session = $this->_getCheckout();
		//$orderid = $session->getJahanpayPaymentRealOrderId();
		...
		@session_start();
		$client = new SoapClient("http://jahanpay.org/directservice?wsdl");
		$res = $client->verification($this->_paymentInst->getConfigData('seller_id'), $_SESSION['jam'], $_SESSION['jau'], $_SESSION['jao'], $_POST + $_GET );
		if($res['result']==1)
		{
			$this->_order->getPayment()->setTransactionId($_SESSION['jau']);
			$this->_order->getPayment()->setLastTransId($_SESSION['jau']);
			// create invoice
			...
			$this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('jahanpay')->__('Payment complete'));
			// send email
			...
			$this->getResponse()->setBody(
				$this->getLayout()
					...
					->toHtml());
			$_SESSION['jam'] = $_SESSION['jau'] = $_SESSION['jao'] = '';
			unset($_SESSION['jam']);
			unset($_SESSION['jau']);
			unset($_SESSION['jao']);
		}
		else
		{
			...
			unset($_SESSION['jau']);
			unset($_SESSION['jao']);
			$this->_redirect('jahanpay/processing/caberror');
		}
	}
    ...
    {
        return Mage::helper('jahanpay')->getPendingPaymentStatus();
    }
}
